    ...
    types::BinaryVector& io_vpdVector)
{
    ...
    if (fd < 0) {
        std::cerr << "Error opening " << m_vpdFilePath << ": " 
                 << strerror(errno) << std::endl;
        return false;
    }

    if (lseek(fd, (m_vpdStartOffset + i_recordECCOffset), SEEK_SET) < 0) {
    ...
    // Write data
    ssize_t bytesWritten = ::write(fd, const_cast<uint8_t*>(&l_recordECCBegin[0]), i_recordECCLength);
    if (bytesWritten < 0) {
