...
/**
 * @brief API to clear VPD of the FRU on DBus
 *
 * This API deletes the VPD for particular interfaces of a FRU on DBus.
 *
 * @param[in] i_objectPath - DBus object path of the FRU.
 * @param[in] i_interfaceMap - Interface and its properties map.
 */
inline void clearVpdOnRemoval(const std::string& i_objectPath,
                              types::InterfaceMap& i_interfaceMap)
{
    std::array<const char*, 0> l_interfaces;

    try
    {
        types::MapperGetObject l_getObjectMap =
            dbusUtility::getObjectMap(i_objectPath, l_interfaces);

        std::vector<std::string> l_vpdRelatedInterfaces{
        ...
        for (const auto& [l_service, l_interfaceList] : l_getObjectMap)
        {
            ...
            {
                if ((l_interface.find("com.ibm.ipzvpd") != std::string::npos) ||
                     ...
                     l_vpdRelatedInterfaces.end()))
                {
                    ...
                    i_interfaceMap.emplace(l_interface,
    ...
    {
        logging::logMessage("Failed to remove FRU VPD with error: " +
