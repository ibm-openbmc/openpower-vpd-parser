                       ...
                       const std::string& i_keywordValue)
{
    ...
    std::error_code l_ec;
    auto l_baseFolder = std::filesystem::path(i_filePath).parent_path();

    if (!l_baseFolder.empty() && !std::filesystem::exists(l_baseFolder, l_ec))
    {
        // ToDo: log only when verbose is enabled
        std::cerr << "Save to file[" << i_filePath << "] failed, Error: ";

        std::string l_errMessage{"Base folder path doesn't exists"};
        if (l_ec)
        {
    ...
    try
    {
        l_outPutFileStream.open(i_filePath, std::ofstream::binary);
    }
    catch (const std::ios_base::failure& l_ex)
    {
        // ToDo: log only when verbose is enabled
        std::cerr << "Failed to open file: " << i_filePath
                  << ", reason: " << l_ex.what() << '\n';
        return l_returnStatus;
    }
    ...
    try
    {
        l_outPutFileStream.write(l_keywordValue.c_str(), l_keywordValue.size());
        l_returnStatus = true;
    }
    catch (const std::ios_base::failure& l_ex)
    {
        // ToDo: log only when verbose is enabled
        std::cerr << "Failed to write to file: " << i_filePath
                  << ", reason: " << l_ex.what() << '\n';
    }

    l_outPutFileStream.close();
    return l_returnStatus;
}
